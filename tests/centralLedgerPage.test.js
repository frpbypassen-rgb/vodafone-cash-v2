...
describe('central ledger page layout', () => {
    ...
    test('operations workspace shows the compact summary strip and the table without the old header', () => {
        ...
        const companiesColumn = html.slice(html.indexOf('class="ops-summary-companies"'), html.indexOf('class="ops-summary-side"'));
        const sideColumn = html.slice(html.indexOf('class="ops-summary-side"'), html.indexOf('id="transactionsTable"'));
        ...
        expect(sideColumn).not.toContain('data-ops-company=');
        expect(sideColumn.indexOf('class="ops-exec-row"')).toBeLessThan(sideColumn.indexOf('class="ops-summary-totals"'));
        const totalsStack = sideColumn.slice(sideColumn.indexOf('class="ops-summary-totals"'));
        ...
        expect(html).toMatch(/\.ops-summary-layout\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*3fr\)\s*minmax\(0,\s*1fr\)/);
        expect(html).toMatch(/\.ops-summary-layout\s*\{[^}]*direction:\s*rtl/);
        expect(html).toMatch(/\.ops-summary-side\s*\{[^}]*flex-direction:\s*column/);
        expect(html).toMatch(/\.ops-summary-totals\s*\{[^}]*flex-direction:\s*column/);
        expect(html).toMatch(/\.ops-exec-row\s*\{[^}]*flex-direction:\s*column/);
        ...
        expect(html).toMatch(/@media \(max-width:\s*991\.98px\)\s*\{[^}]*\.ops-summary-layout\s*\{\s*grid-template-columns:\s*1fr;\s*\}/);
        ...
        expect(html.indexOf('ops-summary-companies')).toBeLessThan(html.indexOf('ops-summary-side'));
