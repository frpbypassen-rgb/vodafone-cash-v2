...
| --- | --- | --- | --- |
| عزل المنظمة: رفض بحث/تحويل عبر `tenantId` | لا | لا رفض. `TENANT_MODE=single` يجعل العلم بلا أثر حتى لو شُغّل بالخطأ. شركة↔وكيل يبقى مسموحًا. | نعم، وللمستقبل `multi` فقط. `FINANCIAL_TENANT_GUARD` افتراضي OFF. |
...
| إلغاء عملية مرتين وقيد `REFUND` | نعم، وهو قديم على مبلغ الرد | مقارنة `ReversalService` مع `main` عند `527c7821`: شرط الحالة غير القابلة للإلغاء لم يتغير، فالنداء الثاني يبقى `success=false` ولا يُرد المبلغ مرة ثانية. الرصيد يتحرك بنفس `costLYD` كما في `main`. الفرق الوحيد على الصف الجديد: `tenantId` اختياري يُنسخ من العملية إن كانت تحمل الحقل. العمليات الملغاة سابقًا لا تُعاد كتابتها. التقارير التي لا تقرأ `tenantId` لا تتغير. تقرير يصفّي بـ `tenantId` قد يرى قيد العكس الجديد إذا كانت العملية الأصلية مختومة. | لا. لا علم يغلّف هذا الختم، وهو لا يغيّر مبلغ الرد. |
