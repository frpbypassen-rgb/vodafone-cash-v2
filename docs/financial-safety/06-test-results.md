...
النتيجة: 25 اختبارًا ناجحًا، 0 فاشل. ستة منها وكل الأعلام محذوفة و`TENANT_MODE=single`: شركة→وكيل، وكيل→شركة، إيداع وخصم خزينة، خصم نقطة البيع والوكيل معًا، تمويل منفذ، وإلغاء مرة واحدة. اختبارات إضافية: غياب Redis لا يرفض التحويل ما دام `FINANCIAL_REDIS_FAIL_CLOSED` مطفأ، و`Idempotency-Key` يُتجاهل ما دام `FINANCIAL_IDEMPOTENCY_ENABLED` مطفأ (خصمان، وصفر مستندات بهذا المفتاح)، وMongo بلا replica set خارج الإنتاج يكمل التحويل، وفشل حفظ التدقيق داخل الجلسة (العلم مفعّل داخل الاختبار فقط) يرجع الرصيد والقيد والتدقيق. اختبارات المنع ورفض Redis تشغّل العلم داخل الاختبار فقط.
...
النتيجة على هذا الفرع (Node 22.14.0، 2026-09-26): 185 مجموعة، 183 ناجحة و2 فاشلة. الاختبارات: 1155، منها 1153 ناجحة و2 فاشلة. منها 7 اختبارات لسكربت القراءة فقط.

نفس الأمر على `main` عند `527c7821` في الجولة السابقة: 183 مجموعة، 181 ناجحة و2 فاشلة. الاختبارات: 1123، منها 1121 ناجحة و2 فاشلة.

الفرق عن `main` (1123 اختبارًا): 32 اختبارًا ناجحًا إضافية. صفر فشل جديد.
