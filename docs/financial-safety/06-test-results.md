...
النتيجة بعد تغليف الأعلام: 16 اختبارًا ناجحًا، 0 فاشل. الاختباران الإضافيان يثبتان أن غياب Redis لا يرفض التحويل ما دام `FINANCIAL_REDIS_FAIL_CLOSED` مطفأ، وأن `Idempotency-Key` يُتجاهل ما دام `FINANCIAL_IDEMPOTENCY_ENABLED` مطفأ (خصمان، وصفر مستندات بهذا المفتاح). اختبارات المنع ورفض Redis تشغّل العلم داخل الاختبار فقط.
...
النتيجة على هذا الفرع (Node 22.14.0): 184 مجموعة، 182 ناجحة و2 فاشلة. الاختبارات: 1139، منها 1137 ناجحة و2 فاشلة.

نفس الأمر على `main` عند `527c7821`: 183 مجموعة، 181 ناجحة و2 فاشلة. الاختبارات: 1123، منها 1121 ناجحة و2 فاشلة.

الفرق: مجموعة واحدة و16 اختبارًا، وكلها ناجحة. صفر فشل جديد.
