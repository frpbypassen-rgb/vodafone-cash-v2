...
## Nginx Reverse Proxy & Load Balancing (Recommended)

For high-availability setups, configure Nginx to load balance incoming traffic across multiple instances of Al-Ahram Pay. 

Using the `ip_hash` directive is critical to ensure that client requests with the same IP address are consistently routed to the same server, preserving WebSocket/Socket.io sessions.

```nginx
# Define the pool of application instances
upstream ahram_backend {
    ip_hash;                  # Session affinity for WebSockets
    server 127.0.0.1:3000;    # App instance 1 (Primary)
    server 127.0.0.1:3001;    # App instance 2 (Secondary)
    keepalive 64;             # Retain connections to backend to minimize latency
}
...
server {
    ...
    location / {
        ...
        # Socket.IO connection limits and timeouts
...
## MongoDB Read Replicas & High Availability

To optimize database read capacity and ensure business continuity, deploy MongoDB in a Replica Set environment. Non-critical database reads (e.g. reports, audit logging views, historical searches) should be routed to secondary nodes to prevent primary overload.

### 1. Connection-Level Routing
To route queries to read replicas automatically at the driver level, configure the `MONGO_URI` with the `readPreference` option set to `secondaryPreferred`:

```bash
# Example Replica Set connection string
MONGO_URI="mongodb://db-primary.example.com:27017,db-secondary-1.example.com:27017,db-secondary-2.example.com:27017/vodafone_cash?replicaSet=rs0&readPreference=secondaryPreferred"
```

### 2. Query-Level Routing in Mongoose
Alternatively, enforce secondary node queries programmatically on read-heavy models (such as reports) by appending the `.read()` option:

```javascript
// Route this query explicitly to secondary members
const reportTransactions = await Transaction.find({ status: 'completed' })
