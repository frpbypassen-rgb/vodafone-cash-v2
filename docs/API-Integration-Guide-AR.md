...
3. متابعة حالة طلب تحويل سابق باستخدام رقم الفاتورة، بما في ذلك اسم ورقم المنفذ عند توفرهما.
4. إرسال إيصال التنفيذ تلقائياً إلى رقم واتساب يرسله الشريك مع الطلب.
...
بيئة الاختبار للشركات:

```text
https://sandbox-api.ahrampay.com
```

هذه بيئة مستقلة ببيانات ومفاتيح اختبار فقط، ولا تتصل بأرصدة أو عمليات الإنتاج.
...
4. عند الاشتباه في تسريب المفتاح يجب أن يقوم مدير الإدارة بتدويره فوراً من صفحة الشركة. تدوير المفتاح يلغي المفتاح القديم لحظياً ويصدر مفتاحاً جديداً؛ حمّل وثيقة الربط الجديدة بعد ذلك.
...
| `amount` | number | نعم | مبلغ التحويل بالجنيه المصري من `100` إلى `50000` لكل عملية |
| `transfer_type` | string | لا | نوع الخدمة. القيمة الافتراضية `vodafone` |
| `whatsapp_number` | string | لا | رقم واتساب الذي يستلم الإيصال الرسمي بعد إتمام التحويل. يُحفظ بصيغة دولية. يقبل `client_phone` كاسم قديم للتوافق فقط |
...
4. عند إرسال `whatsapp_number` يرسل النظام الإيصال الرسمي تلقائياً إلى الرقم بعد اكتمال العملية وتوفر إثبات التنفيذ.
5. يظهر `executor_name` و`executor_number` متى سجل النظام بيانات المنفذ. قد يكون `executor_number` بقيمة `null` في لحظة إنشاء الطلب؛ لا تخمّنه ولا تعتبر ذلك خطأ.
6. في هذه النسخة لا يوجد `Idempotency-Key` مفعل على Merchant API. لذلك يجب على نظام الشركة عدم إعادة إرسال نفس الطلب تلقائياً إذا استلم `invoice_number`.
...
```json
{
  ...
  "data": {
    ...
    "receipt_whatsapp_number": "201108172258",
    "executor_name": "أحمد - مجموعة التنفيذ",
...
|---:|---|
| `200` | الطلب ناجح |
| `400` | بيانات غير صحيحة، نوع تحويل غير مدعوم، رقم واتساب غير صالح، مبلغ خارج `100`–`50000`، أو رصيد غير كاف |
...
sequenceDiagram
    ...
    API-->>Partner: الحالة النهائية أو الحالية + بيانات المنفذ عند توفرها
    API->>Partner: إرسال إيصال واتساب عند completed إذا أُرسل whatsapp_number
```
...
7. حفظ رقم واتساب الخاص بالإيصال في سياق الطلب فقط، وعدم استخدامه لأي رسائل تسويقية.
...
| `1.1` | `2026-09-01` | إضافة حد العملية 100–50000، رقم واتساب للإيصال، رقم المنفذ في الاستعلام، وآلية تدوير مفتاح API من الإدارة |
