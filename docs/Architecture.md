...
sequenceDiagram
    ...
    participant STS as 💼 Settlement Service
    participant DB as 💾 MongoDB
    participant WS as 💰 Wallet Service
    participant L as 📒 Ledger
    participant TB as 🤖 Telegram Bot / WebSockets

    AD->>STS: POST /settlement/create { entityId, period }
    STS->>DB: Query completed transactions in period
    STS->>STS: Calculate net total, commission, and due amount
    STS->>DB: Save Settlement record (status: pending)
    STS-->>AD: Return settlement preview

    Note over AD,STS: Approval Phase
    AD->>STS: POST /settlement/approve/:id
    STS->>DB: START TRANSACTION
    STS->>DB: Update Settlement status -> approved
    STS->>WS: adjustBalance(entityId, amount)
    WS->>DB: findOneAndUpdate({ $inc: balance })
    WS->>L: Create SETTLEMENT entry
    STS->>DB: COMMIT TRANSACTION
    STS-->>AD: Settlement approved successfully
    STS->>TB: Broadcast settlement completion (Socket.IO / Bot)
```
