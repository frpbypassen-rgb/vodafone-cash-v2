...
const postTransferWizard = new Scenes.WizardScene(
    ...
    async (ctx) => {
        ...
        if (ctx.wizard.state.step === 'awaiting_amount') {
            ...
            try {
                ...
                let clientTier = 1, currentBalance = 0, creditLimit = 0;

                if (ctx.wizard.state.isMainBot) {
                    clientTier = userObj.tier || 1;
                    currentBalance = parseFloat(userObj.balance) || 0;
                    creditLimit = Math.abs(parseFloat(userObj.creditLimit) || 0);
                } else {
                    clientTier = companyObj.tier || 1;
                    currentBalance = parseFloat(companyObj.balance) || 0;
                    creditLimit = Math.abs(parseFloat(companyObj.creditLimit) || 0);
                }
                ...
                if (clientTier === 2) baseRate = set.rateLevel2 || 6.45;
                if (clientTier === 3) baseRate = set.rateLevel3 || 6.50;
                ...
                await editPrompt(ctx, '❌ حدث خطأ أثناء التواصل مع الخادم.', {});
                return ctx.scene.leave();
            }
        }
    },
    ...
    async (ctx) => {
        ...
        if (ctx.callbackQuery.data === 'confirm_post_tx') {
            try {
                const { transferType, accountName, accountNumber, idCardImage, amount, costLYD, nationalId, governorate } = ctx.wizard.state;
                ...
                if (error.response && error.response.status === 429) {
                    await editPrompt(ctx, `⚠️ <b>تحذير أمني:</b> يرجى الانتظار قليلاً قبل المحاولة.`, {});
                    ...
                    await editPrompt(ctx, '❌ حدث خطأ أثناء الإرسال.', {});
                }
            }
            return ctx.scene.leave();
        }
